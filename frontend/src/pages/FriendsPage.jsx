    ...
    >
      ...
      <main className="relative z-10 flex flex-row gap-10 pt-10 px-10 pb-20">
        <div className="flex-1 max-w-2xl">
          ...
          <div className="flex flex-col gap-3 mb-10">
            ...
            {friends.map(friend => (
              <div key={friend.id || friend._id}
                ...
                style={{ background: 'rgba(255,255,255,0.7)', border: `1px solid ${P.lavLight}`, backdropFilter: 'blur(8px)' }}
              >
          ...
          <div className="flex flex-col gap-4">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)}
              ...
              className="w-80 px-5 py-3 rounded-full focus:outline-none shadow-sm transition-all"
              style={{ border: `1.5px solid ${P.lavLight}`, fontFamily: 'inherit', color: P.navy, background: 'rgba(255,255,255,0.9)' }} />

            {searchResults.length > 0 && (
              <div className="mt-2 flex flex-col gap-3">
                {searchResults.map(user => (
                  <div key={user._id} className="flex items-center justify-between w-80 p-3 rounded-2xl"
                    style={{ background: 'rgba(255,255,255,0.8)', border: `1px solid ${P.lavLight}` }}>
                    <div className="flex items-center gap-3">
        ...
        <div className="w-64 flex-shrink-0">
          <div className="sticky top-10 rounded-3xl p-6 shadow-md"
            style={{ background: 'rgba(255,255,255,0.75)', border: `1px solid ${P.lavLight}`, backdropFilter: 'blur(10px)' }}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-base font-bold uppercase tracking-wide" style={{ color: P.navy }}>Top Studdle</h3>
              <button onClick={() => navigate("/leaderboard")} className="text-xs font-bold"
