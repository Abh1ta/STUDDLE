      ...
      {topUsers.map((u, index) => (
        <div key={u._id} className="flex items-center justify-between group">
          <div className="flex items-center gap-3">
            <span className="text-xs font-black text-blue-300 w-4">{index + 1}</span>
            <img
              src={u.avatar_url || catAvatar}
              className="w-8 h-8 rounded-full border border-blue-100 object-cover"
              alt=""
            />
            <span className="text-sm font-bold text-blue-950 truncate w-24">
              {u.username}
            </span>
          </div>
          <div className="text-right">
            <span className="text-xs font-black text-blue-600 block">
              {Number(u.xp || 0).toFixed(1)}
            </span>
            <span className="text-[9px] text-blue-300 font-bold uppercase">XP</span>
          </div>
        </div>
      ))}
    </div>
...
function FriendsPage() {
  ...
  return (
    ...
    >
      ...
      <main
        className="relative z-10 mt-10 flex flex-row gap-12 pl-32 pr-16"
      ...
      >
        ...
        <div className="w-72 flex-shrink-0">
          <div className="bg-white/60 backdrop-blur-md border border-blue-100 p-6 rounded-[30px] shadow-sm sticky top-28">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-semibold text-blue-950 uppercase tracking-tight">
                Top Studdle
