  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        navigate('/'); // Te trimite la pagina principală după logare
      }
    } catch (err) {
  ...
  return (
    <div style={{
      ...
      <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.5rem 5rem 1.5rem 2.5rem' }}>
        <img src={logoStuddle} alt="Studdle logo" style={{ height: '4rem', width: 'auto', objectFit: 'contain' }} />
      </nav>
      ...
      {/* ILUSTRAȚIA */}
      <div style={{ position: 'absolute', bottom: 40, left: '50%', transform: 'translateX(-50%)', width: '550px', pointerEvents: 'none', zIndex: 1 }}>
